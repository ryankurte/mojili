...
router.post('/', function(req, res) {
	...
	if(!url) {
		return res.json({message: 'url argument required'});
	}

	if(!validator.isURL(url, {require_protocol: true})) {
		return res.json({message: 'url does not appear to be a url'});
	}
	...
	.then(function(newUrl) {
		return res.json({message: 'created url', real_url: url, emoji_url: newUrl})
